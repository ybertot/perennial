...
Definition staged_inv `{!invG Σ, !crashG Σ, !stagedG Σ} (N: namespace) (γ γ': gname) (Q: iProp Σ) : iProp Σ :=
...
Global Instance staged_contractive N γ γ' : Contractive (staged_inv N γ γ').
Proof. rewrite /staged_inv. solve_contractive. Qed.

Global Instance staged_ne N γ γ': NonExpansive (staged_inv N γ γ').
Proof. apply contractive_ne, _. Qed.

Global Instance staged_proper N γ γ' : Proper ((⊣⊢) ==> (⊣⊢)) (staged_inv N γ γ').
Proof. apply ne_proper, _. Qed.

Global Instance staged_persistent N γ γ' P : Persistent (staged_inv N γ γ' P).
...
Lemma staged_inv_iff N γ γ' P Q : ▷ □ (P ↔ Q) -∗ staged_inv N γ γ' P -∗ staged_inv N γ γ' Q.
Proof.
  iIntros "#HPQ". iApply inv_iff. iNext. iAlways. iSplit.
  - iIntros "H". iDestruct "H" as (?? P0 P0') "(?&?&?&#HP0&Hcase)". iExists _, _, P0, P0'. iFrame.
    iAlways. iIntros. iDestruct ("HP0" with "[$] [$]") as "(?&?)". iFrame. by iApply "HPQ".
  - iIntros "H". iDestruct "H" as (?? P0 P0') "(?&?&?&#HP0&Hcase)". iExists _, _, P0, P0'. iFrame.
    iAlways. iIntros. iDestruct ("HP0" with "[$] [$]") as "(?&?)". iFrame. by iApply "HPQ".
Qed.

Lemma staged_inv_alloc N E P Q Qr:
  ▷ Q ∗ □ (C -∗ Q -∗ P ∗ Qr) ={E}=∗
  ∃ γ γ', staged_inv N γ γ' P ∗ staged_value N γ Q Qr ∗ staged_pending γ'.
Proof.
  ...
  { econstructor. }
  iIntros "HQP".
  iMod (inv_alloc N E _ with "[HQP H1]") as "HI"; last first.
  { iModIntro. iExists γ, γ'. iFrame "H". iSplitL "HI".
    - iApply "HI".
    - iExists γprop, γprop'. iFrame. iFrame "#".
  }
  iNext. iExists γprop, γprop', Q, Qr. iFrame. iFrame "#".
  iDestruct "HQP" as "(HQ&$)". by iLeft.
Qed.

Lemma staged_inv_open E N γ γ' P Q Qr:
  ↑N ⊆ E → staged_inv N γ γ' P ∗ staged_value N γ Q Qr
           ={E,E∖↑N}=∗ (▷ ▷ Q ∗ (∀ Q' Qr', ▷ Q' ∗ □ (C -∗ Q' -∗ P ∗ Qr') ={E∖↑N,E}=∗ staged_value N γ Q' Qr')) ∨
                       (▷ ▷ Qr ∗ C ∗ |={E∖↑N, E}=> emp).
Proof.
  iIntros (?) "(#Hinv&Hval)".
...
Lemma staged_inv_weak_open E N γ γ' P:
  ↑N ⊆ E → (staged_inv N γ γ' P ∗ staged_pending γ' ∗ C ={E}=∗ ▷ P)%I.
Proof.
  ...
  iAssert (▷ (P ∗ Qrsaved))%I with "[HQ]" as "(HQ&HP)".
  { iNext. iApply "HQ0"; by iFrame. }
  iMod (pending_upd_done with "[$]") as "#Hdone".
  iMod ("Hclose" with "[HP Hγ']").
