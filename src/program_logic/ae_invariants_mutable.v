...
Definition ae_inv_mut_def `{!invG Σ} (k: nat) sch (Ps : list (iProp Σ)) : iProp Σ :=
    ∃ i, ⌜i ∈ AE' (S k)⌝ ∧ ownI k i sch (list_to_vec Ps).
...
Definition ae_inv_mut_full_def `{!invG Σ} (k: nat) sch (Qs Ps : list (iProp Σ)) : iProp Σ :=
    ∃ i, ⌜i ∈ AE' (S k)⌝ ∗ ownI k i sch (list_to_vec Ps) ∗ ownI_mut k i (1/2)%Qp (list_to_vec Qs).
...
Section ae_inv_mut.
  ...
  Definition ae_shift k E P P' Q :=
    match k with
     | O => P ==∗ ◇ (P' ∗ Q)
     | S k' => P -∗ |k'={E}=> P' ∗ Q
    end%I.

  Lemma ae_inv_mut_full_acc k E sch Qs Qs' Ps R :
    ae_inv_mut_full k sch Qs Ps -∗
    ae_shift k E
             (bi_schema_interp k (bi_later <$> Ps) (bi_later <$> Qs) sch)
             (bi_schema_interp k (bi_later <$> Ps) (bi_later <$> Qs') sch)
             R -∗
    |k={E}=> ae_inv_mut_full k sch Qs' Ps ∗ R.
  Proof.
    rewrite uPred_fupd_level_eq /uPred_fupd_level_def ae_inv_mut_full_eq.
    ...
    - iDestruct (wsat_le_acc (S k) with "Hw") as "(Hw&Hclo)"; first lia.
      rewrite /ae_shift. rewrite uPred_fupd_level_eq /uPred_fupd_level_def.
      rewrite ownE_op //.
  ...
  Lemma ae_inv_mut_acc k E sch Ps R :
    ae_inv_mut k sch Ps -∗
    (∀ Qs, ae_shift k E
             (bi_schema_interp k (bi_later <$> Ps) (bi_later <$> Qs) sch)
             (bi_schema_interp k (bi_later <$> Ps) (bi_later <$> Qs) sch)
    ...
    |k={E}=> R.
  Proof.
    rewrite uPred_fupd_level_eq /uPred_fupd_level_def ae_inv_mut_eq.
  ...
  Proof.
    rewrite uPred_fupd_level_eq ?ae_inv_mut_eq ?ae_inv_mut_full_eq. iIntros "HP [Hw $]".
    iMod (ownI_alloc (.∈ AE' (S k)) sch k (list_to_vec Ps) (list_to_vec Qs)
            with "[HP $Hw]")
